	...
	, m_bShowCubes(true)
{
	...
	{
		if (!_stricmp(argv[i], "-windowed"))
		{
			m_bRunOnMainWindow = true;
		}
		else if (!_stricmp(argv[i], "-gldebug"))
		{
			m_bDebugOpenGL = true;
		}
		...
		else if (!_stricmp(argv[i], "-nocompositor"))
		{
...
void CMainApplication::convert_hmd_Matrix(float* lookforward, float*  lookup, float*  lookright, float*  shiftPos)
{
...
void CMainApplication::RenderDistortion_left()
{
	glDisable(GL_DEPTH_TEST);
	glViewport(0, 0, m_nWindowWidth, m_nWindowHeight);
	...
	//pixel_data = new unsigned char[1024 * 1024 * 4];
	...
	//	{
	//		fputc(pixel_data[x * 1024 * 4 + y * 4 + 0], f);   // 0 .. 255
	//		fputc(pixel_data[x * 1024 * 4 + y * 4 + 1], f); // 0 .. 255
	//		fputc(pixel_data[x * 1024 * 4 + y * 4 + 2], f);  // 0 .. 255
	//	}
	//}
	//	
	//fclose(f);
...
void CMainApplication::RenderDistortion_right()
{
